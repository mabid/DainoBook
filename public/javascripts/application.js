...
all_digits = function(numValue)
{
    var regexNum = new RegExp('^[0-9]+$')
    return regexNum.test(numValue);
}
...
onKeyPress: function(event) {
		...
		var new_size = 40 + (this.element.value.length * 7);
        if (new_size < 300) {
			this.set_input_size(new_size);
        } else {
			this.set_input_size(300);
        }
...
setOptions: function(options) {
        this.options = Object.extend({
            ...
            selector: function(instance) {
                ...
                ret.length < instance.options.choices; i++) {
                    ...
                    while (foundPos != -1) {
                        ...
                        if (foundPos == 0 && elem_name.length != entry.length) {
                            ...
                            + "<div id='value' style='display:none;'>" + elem_name + "</div>" + "</li>"
                            );
                            break;
                ...
                if( !ret.size() && all_digits(entry) )
                {
                    ret.push("<li>" + "<div>" + entry + "</div><div id='value' style='display:none;'>"+entry+"</div></li>");
                }
...
HiddenInput = Class.create({
 ...
 Token = Class.create({
    ...
    select: function() {
        ...
        if( this.hidden_input.element != null )
...
addContactToList = function(item, value) {
    ...
    Builder.node('span',
    Builder.node('span', {},
    [Builder.node('input', { type: "hidden", name: "to",id:'id_to',
        value: value || item.down('div#value').innerHTML
    }),
	value || item.down('div#value').innerHTML,
